...
#include "VenomSerial.h"

#define VN_PAGE_SIZE 4096

typedef struct VMNode {
    int KeyNum;/* 关键字个数 */
    KeyType Key[VN_M + 1];/* 内部节点从1开始(实际上0的位置存储了第一棵子树的最小值)，树叶节点从0开始 */
    uint32_t Children[VN_M + 1];/* 存储M个孩子节点 */
} VMNode;

typedef struct VMVector {
    uint32_t count;
    uint32_t capacity;
    uint32_t rootIdx;
    VMNode *tree;//树的开始
...
static KeyType Unavailable = 0xFFFFFFFF;

const uint32_t VM_INDEX_MAX = 0xFFFFFFFF;

static VMVector *VMVectorInit() {
    uint32_t capacity = VN_PAGE_SIZE;
    VMVector *vector = calloc(1, capacity);
    vector->count = 0;
    vector->capacity = capacity;
    vector->tree = (VMNode *)(vector + 1);
    vector->root = vector->tree + vector->rootIdx;
    return vector;
}
//
//static void VenomSerialPut(VenomSerial *serial, const KeyType key) {
//    if (serial != NULL) {
//        serial->root = Insert(serial->root, key);
//        serial->count += 1;
//    }
//}

/* 生成节点并初始化 */
static VMNode *MallocNewNode() {
    VMTree NewNode = malloc(sizeof(struct VMNode));
    if (NewNode == NULL)
    exit(EXIT_FAILURE);
    ...
    while (i < VN_M + 1){
        ...
        NewNode->Children[i] = VM_INDEX_MAX;
        i++;
    }
    NewNode->KeyNum = 0;
...
/* 分裂节点 */
static VMTree SplitNode(Position Parent,Position X,int i){
    int j,k,Limit;
    Position NewNode;
...
static VMTree RecursiveInsert(VMTree T,KeyType Key,int i,VMTree Parent){
    int j,Limit;
    Position Sibling;
...
static VMTree RecursiveRemove(VMTree T,KeyType Key,int i,VMTree Parent){
    ...
    else if (Parent != NULL && T->Children[0] != NULL && T->KeyNum < VN_LIMIT_M_2)
    NeedAdjust = 1;
...
static void RecursiveTravel(VMTree T,int Level){
    int i;
    if (T != NULL){
        printf("  ");
